WITH t0 AS (
  SELECT
    t2.ps_partkey AS ps_partkey,
    t2.ps_suppkey AS ps_suppkey,
    t2.ps_availqty AS ps_availqty,
    t2.ps_supplycost AS ps_supplycost,
    t2.ps_comment AS ps_comment,
    t3.p_partkey AS p_partkey,
    t3.p_name AS p_name,
    t3.p_mfgr AS p_mfgr,
    t3.p_brand AS p_brand,
    t3.p_type AS p_type,
    t3.p_size AS p_size,
    t3.p_container AS p_container,
    t3.p_retailprice AS p_retailprice,
    t3.p_comment AS p_comment
  FROM main.partsupp AS t2
  JOIN main.part AS t3
    ON t3.p_partkey = t2.ps_partkey
)
SELECT
  t1.p_brand,
  t1.p_type,
  t1.p_size,
  t1.supplier_cnt
FROM (
  SELECT
    t0.p_brand AS p_brand,
    t0.p_type AS p_type,
    t0.p_size AS p_size,
    COUNT(DISTINCT t0.ps_suppkey) AS supplier_cnt
  FROM t0
  WHERE
    t0.p_brand <> 'Brand#45'
    AND NOT t0.p_type LIKE 'MEDIUM POLISHED%'
    AND t0.p_size IN (CAST(49 AS TINYINT), CAST(14 AS TINYINT), CAST(23 AS TINYINT), CAST(45 AS TINYINT), CAST(19 AS TINYINT), CAST(3 AS TINYINT), CAST(36 AS TINYINT), CAST(9 AS TINYINT))
    AND (
      NOT t0.ps_suppkey IN (
        SELECT
          t4.s_suppkey
        FROM (
          SELECT
            t5.s_suppkey AS s_suppkey,
            t5.s_name AS s_name,
            t5.s_address AS s_address,
            t5.s_nationkey AS s_nationkey,
            t5.s_phone AS s_phone,
            t5.s_acctbal AS s_acctbal,
            t5.s_comment AS s_comment
          FROM main.supplier AS t5
          WHERE
            t5.s_comment LIKE '%Customer%Complaints%'
        ) AS t4
      )
    )
  GROUP BY
    1,
    2,
    3
) AS t1
ORDER BY
  t1.supplier_cnt DESC,
  t1.p_brand ASC,
  t1.p_type ASC,
  t1.p_size ASC