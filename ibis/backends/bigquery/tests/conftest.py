...
class TestConf(UnorderedComparator, BackendTest, RoundAwayFromZero):
    ...
    def _load_data(data_dir: Path, script_dir: Path, **_: Any) -> None:
        ...
        job_args = [
            ...
            (
                client.load_table_from_file,
                io.BytesIO(data_dir.joinpath("functional_alltypes.csv").read_bytes()),
                functional_alltypes_parted,
                bq.LoadJobConfig(
                    schema=ibis_schema_to_bq_schema(TEST_TABLES["functional_alltypes"]),
                    write_disposition=write_disposition,
        ...
        with concurrent.futures.ThreadPoolExecutor() as e:
            for future in concurrent.futures.as_completed(
                e.submit(make_job, *args, job_config=job_config)
                for *args, job_config in job_args
            ):
                future.result()
