...
class Backend(SQLBackend, CanCreateDatabase, UrlFromPath):
    ...
    def raw_sql(self, query: str | sg.Expression, **kwargs: Any) -> Any:
        ...
        cur = self.con.cursor(ArrowCursor)
        try:
    ...
    def _safe_raw_sql(self, query, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            query = query.sql(self.dialect)
        with self.con.cursor(unload=True) as cur:
            yield cur.execute(query, *args, **kwargs)

    def list_catalogs(self, like: str | None = None) -> list[str]:
        with self.con.cursor() as cur:
            out = cur.catalogs().as_arrow()
    ...
    def list_databases(
        ...
        with self.con.cursor() as cur:
            out = cur.schemas(catalog_name=catalog or self.current_catalog).as_arrow()
    ...
    def _fetch_from_cursor(self, cursor, schema: sch.Schema) -> pd.DataFrame:
        if (table := cursor.as_arrow()) is None:
