...
def spark_client(data_directory):
    pt = pytest.importorskip('pyspark.sql.types')
    ibis_spark = pytest.importorskip('ibis.spark.api')

    global _SPARK_CLIENT
    if _SPARK_CLIENT is None:
        client = ibis_spark.connect()
        ...
        _SPARK_CLIENT = client

    assert _SPARK_CLIENT is not None
