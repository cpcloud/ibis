...
@execute_node.register(
    ops.Aggregation, pd.DataFrame, tuple, tuple, tuple, tuple, tuple
)
def execute_aggregation_dataframe(
    op, data, metrics, by, having, predicates, sort_keys, scope=None, **kwargs
):
    ...
    if having:
        ...
        predicate = functools.reduce(
            operator.and_,
            (execute(having, scope=new_scope, **kwargs) for having in having),
        )
...
@execute_node.register(
    ops.Contains, pd.Series, (collections.Sequence, collections.Set)
)
...
@execute_node.register(
    ops.NotContains, pd.Series, (collections.Sequence, collections.Set)
)
