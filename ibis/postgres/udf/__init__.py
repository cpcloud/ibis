...
from ...sql.alchemy import _to_sqla_type
from ..compiler import PostgreSQLExprTranslator, PostgresUDFNode, add_operation
...
class PostgresUDFError(Exception):
    pass
...
def sa_type_to_postgres_str(sa_type):
    ...
    return sa_type().compile(dialect=sa.dialects.postgresql.dialect())


def ibis_to_postgres_str(ibis_type):
    """Map an ibis DataType to a Postgres string."""
...
def existing_udf(name, input_types, output_type, schema=None, parameters=None):
    ...
    schema: Optional[str]
        Optionally specify the Postgres schema that the UDF is defined in.
    parameters: Optional[List[str]]
        The names to the arguments of the UDF
    ...
    if parameters is None:
        parameters = ["v{:d}".format(i) for i in range(len(input_types))]
    elif len(input_types) != len(parameters):
        raise ValueError(
            (
                "Length mismatch in arguments to existing_udf: "
                "len(input_types)={:d}, len(parameters)={:d}"
...
def udf(
    ...
    name : Optional[str]
        The name for the UDF to be defined in database
