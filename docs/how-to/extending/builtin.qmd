...
ibis.set_backend(con)  # <1>
```

1. Set `con` to be the default backend for this session
...
expr = (
   basics_ratings.mutate(
      kurt=kurtosis(_.num_votes).over(group_by="title_type")
   )
