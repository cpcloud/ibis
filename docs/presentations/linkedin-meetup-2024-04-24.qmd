...
- The API
- E.g., `StringValue`, `IntegerColumn`, `Table`
- Methods like `sum()`, `split()`, `join`
...
::: {.fragment}
::: {.callout-warning}
## Operations are not part of the public API
:::
:::
...
expr = (
    penguins
    .group_by("species")
    .agg(avg_bill_length_mm=_.bill_length_mm.mean())
)
...
![* Rewrites <br/>* bottom up traversal storing intermediate outputs <br/>* handoff the output to sqlglot.](./magic.gif)
...
- (Heavily) massage the output (pandas 👀👀)
